...
function throwDefaultError(messageId: string, messagePrefix: string, error: object) {
...
export default function errorHandler(error: any) {
  if (error.response) {
    ...
    if (error.response.data) {
      ...
      //   throw new ServiceError(code, message, error);
      // }
    }
    ...
    if (error.message.indexOf("timeout of ") === 0) {
      throw new ServiceError("SYSMSG-SERVICE-TIMEOUT", null, error);
      // server unavaliable
